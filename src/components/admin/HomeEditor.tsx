...
import { Save, Loader2, AlertCircle, CheckCircle, Copy, ClipboardCheck } from 'lucide-react';
...
export default function HomeEditor() {
  ...
  const SQL_MIGRATION = `-- Run this in your Supabase SQL Editor to create the home_content table:
...
CREATE POLICY "Allow admin manage home_content" ON public.home_content
  ...
  const handleReload = async () => {
    ...
    try {
      ...
      if (error) {
        if (error.code === 'PGRST205' || error.message?.includes('home_content')) {
          setMissingTable(true);
          return;
        }
        throw error;
    ...
    return (
      <div className="p-6 bg-amber-500/10 border border-amber-500/20 rounded-2xl text-amber-400 space-y-4 max-w-2xl">
        ...
        <p className="text-xs text-gray-300 leading-relaxed">
          The table <code>home_content</code> does not exist in your Supabase database. Please copy the SQL below and execute it inside the <strong>SQL Editor</strong> on your Supabase dashboard to create the table.
        </p>
