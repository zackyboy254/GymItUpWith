...
import { ArrowUpRight, Award, Bike, Brain, Check, ChevronLeft, ChevronRight, Dumbbell, Flame, HeartPulse, Mountain, Network, PersonStanding, Sparkles, Star, Target, Trophy, Users, Waves } from 'lucide-react';
...
export default function HomePage() {
  ...
  useEffect(() => {
    async function loadContent() {
      try {
        const { data } = await supabase.from('home_content').select('*').eq('status', 'active').order('id', { ascending: true }).limit(1).maybeSingle();
        if (data) { setHeroTitle(data.title || 'Grow strong. Live better.'); setHeroSubtitle(data.subtitle || 'A premium fitness community for body transformation, accountability, and a life that feels fully switched on.'); }
      } catch { /* Static content keeps the page useful when Supabase is unavailable. */ }
      try { const { data } = await supabase.from('carousel_slides').select('*').eq('status', 'active').order('order', { ascending: true }); if (data?.length) setCarouselImages(data.map((slide: { image_id: string }) => slide.image_id)); } catch { /* Use local fallbacks. */ }
      try { const { data } = await supabase.from('quotes').select('text, author').eq('status', 'active').limit(1); if (data?.[0]) setQuote(data[0]); } catch { /* Use the static quote. */ }
      try {
        const { data } = await supabase.from('programs').select('title,description,duration,difficulty,featured,display_order,status').eq('status', 'active').order('display_order', { ascending: true });
        if (data?.length) {
          ...
          setPrograms(data.map((program: { title: string; description: string; duration: string | null; difficulty: string | null; featured: boolean | null }) => ({ title: program.title, description: program.description, level: program.difficulty || 'All levels', duration: program.duration || 'Flexible', icon: iconFor(program.title), featured: Boolean(program.featured) })));
        }
      } catch { /* The static program cards keep older installations functional. */ }
    }
    loadContent();
  }, []);
    ...
    <main className="overflow-hidden bg-[#08100f] text-white">
      <section className="relative flex min-h-[92vh] items-end overflow-hidden">
        ...
        <div className="relative z-10 mx-auto grid w-full max-w-7xl gap-12 px-5 pb-20 pt-40 sm:px-8 lg:grid-cols-[1.1fr_.9fr] lg:px-12">
          <div className="max-w-3xl self-center"><div className="mb-7 inline-flex items-center gap-2 rounded-full border border-[#d8f36b]/30 bg-[#d8f36b]/10 px-4 py-2 text-xs font-bold uppercase tracking-[.24em] text-[#d8f36b]"><Sparkles className="h-4 w-4" /> The Grow Fit community</div><h1 className="max-w-3xl text-6xl font-black uppercase leading-[.9] tracking-[-.05em] sm:text-8xl">{heroTitle}</h1><p className="mt-8 max-w-xl text-lg leading-relaxed text-white/75 sm:text-xl">{heroSubtitle}</p><div className="mt-9 flex flex-col gap-3 sm:flex-row"><button onClick={openModal} className="inline-flex items-center justify-center gap-2 rounded-full bg-[#d8f36b] px-6 py-4 text-sm font-black uppercase tracking-wider text-[#15200e] transition hover:-translate-y-1 hover:bg-white">Join the Community <ArrowUpRight className="h-4 w-4" /></button><Link href="#programs" className="inline-flex items-center justify-center gap-2 rounded-full border border-white/25 bg-white/10 px-6 py-4 text-sm font-black uppercase tracking-wider backdrop-blur transition hover:bg-white/20">Start Your Transformation <ChevronRight className="h-4 w-4" /></Link></div><div className="mt-12 flex flex-wrap gap-x-8 gap-y-4 text-sm text-white/65"><span><strong className="text-white">27K+</strong> members growing</span><span><strong className="text-white">98%</strong> success rate</span><span><strong className="text-white">60+</strong> weekly sessions</span></div></div>
          <div className="hidden self-end justify-self-end lg:block"><div className="w-72 border-l border-[#d8f36b]/60 pl-5"><p className="text-xs font-bold uppercase tracking-[.25em] text-[#d8f36b]">This week at Grow Fit</p><p className="mt-3 text-2xl font-bold leading-tight">{quote.text}</p><p className="mt-4 text-sm text-white/50">{quote.author}</p></div></div>
        </div>
        <div className="absolute bottom-7 right-6 z-10 flex gap-2">{carouselImages.map((_, index) => <button key={index} onClick={() => setCurrentSlide(index)} aria-label={`Show hero image ${index + 1}`} className={`h-1.5 rounded-full transition-all ${index === currentSlide ? 'w-12 bg-[#d8f36b]' : 'w-5 bg-white/35'}`} />)}</div>
      </section>

      <section id="programs" className="mx-auto max-w-7xl px-5 py-24 sm:px-8 lg:px-12"><ScrollReveal><div className="flex flex-col justify-between gap-6 md:flex-row md:items-end"><div><p className="eyebrow">Choose your next chapter</p><h2 className="section-title">Challenges that <span>change you.</span></h2></div><p className="max-w-md text-white/55">Coaching that respects your starting point, your schedule, and the person you are becoming.</p></div></ScrollReveal><div className="mt-12 grid gap-4 md:grid-cols-2 lg:grid-cols-3">{programs.map((program) => { const Icon = program.icon; return <ScrollReveal key={program.title} className={program.featured ? 'lg:col-span-2' : ''}><article className={`group relative h-full overflow-hidden rounded-2xl border p-6 transition duration-300 hover:-translate-y-2 ${program.featured ? 'border-[#d8f36b]/40 bg-[#d8f36b] text-[#15200e]' : 'border-white/10 bg-white/[.06] hover:border-[#d8f36b]/40'}`}><div className="flex items-start justify-between"><div className={`flex h-11 w-11 items-center justify-center rounded-xl ${program.featured ? 'bg-[#15200e]/10' : 'bg-[#d8f36b]/10 text-[#d8f36b]'}`}><Icon className="h-5 w-5" /></div><span className={`text-xs font-bold uppercase tracking-wider ${program.featured ? 'text-[#15200e]/60' : 'text-white/45'}`}>{program.duration}</span></div><h3 className="mt-12 text-2xl font-black uppercase tracking-tight">{program.title}</h3><p className={`mt-3 max-w-lg text-sm leading-relaxed ${program.featured ? 'text-[#15200e]/70' : 'text-white/55'}`}>{program.description}</p><div className="mt-6 flex items-center justify-between border-t pt-4 text-xs font-bold uppercase tracking-wider" style={{ borderColor: program.featured ? 'rgba(21,32,14,.15)' : 'rgba(255,255,255,.1)' }}><span>{program.level}</span><button onClick={openModal} className="inline-flex items-center gap-1">Join program <ArrowUpRight className="h-4 w-4" /></button></div></article></ScrollReveal> })}</div></section>

      <section className="border-y border-white/10 bg-[#101b18] px-5 py-24 sm:px-8 lg:px-12"><div className="mx-auto max-w-7xl"><ScrollReveal><p className="eyebrow">Beyond the gym</p><h2 className="section-title">Meet us <span>outside.</span></h2><p className="mt-5 max-w-xl text-white/55">The best kind of progress has a little fresh air, a few new friends, and a story worth telling.</p></ScrollReveal><div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-5">{activities.map(([title, description, schedule, Icon, image]) => { const ActivityIcon = Icon as typeof Bike; return <ScrollReveal key={title as string}><article className="group overflow-hidden rounded-2xl border border-white/10 bg-[#08100f]"><div className="relative h-44 overflow-hidden"><img src={image as string} alt="" className="h-full w-full object-cover transition duration-700 group-hover:scale-110" /><div className="absolute inset-0 bg-gradient-to-t from-[#08100f] to-transparent" /></div><div className="p-5"><div className="flex items-center justify-between"><h3 className="font-bold">{title as string}</h3><ActivityIcon className="h-4 w-4 text-[#d8f36b]" /></div><p className="mt-3 text-sm leading-relaxed text-white/55">{description as string}</p><p className="mt-5 text-xs font-bold uppercase tracking-wider text-[#d8f36b]">{schedule as string}</p><button onClick={openModal} className="mt-4 text-sm font-bold text-white/75 hover:text-[#d8f36b]">Join activity <ArrowUpRight className="ml-1 inline h-4 w-4" /></button></div></article></ScrollReveal> })}</div></div></section>

      <section className="mx-auto grid max-w-7xl gap-14 px-5 py-24 sm:px-8 lg:grid-cols-[.8fr_1.2fr] lg:px-12"><ScrollReveal><p className="eyebrow">Real people. Real momentum.</p><h2 className="section-title">Proof is <span>personal.</span></h2><p className="mt-5 max-w-sm text-white/55">Every transformation starts with one honest decision and gets stronger with a community behind it.</p><button onClick={openModal} className="mt-8 inline-flex items-center gap-2 rounded-full border border-[#d8f36b]/40 px-5 py-3 text-sm font-bold text-[#d8f36b] hover:bg-[#d8f36b] hover:text-[#15200e]">Join the next challenge <ArrowUpRight className="h-4 w-4" /></button></ScrollReveal><TestimonialCarousel /></section>

      <section className="relative overflow-hidden bg-[#d8f36b] px-5 py-24 text-[#15200e] sm:px-8 lg:px-12"><div className="absolute -right-10 -top-20 text-[20rem] font-black leading-none opacity-[.08]">GF</div><div className="relative mx-auto grid max-w-7xl gap-12 lg:grid-cols-[1fr_1fr]"><ScrollReveal><p className="eyebrow text-[#15200e]/60">More than a workout</p><h2 className="section-title text-[#15200e]">Build your <span>network.</span></h2><p className="mt-6 max-w-lg text-lg leading-relaxed text-[#15200e]/75">Train with people from different industries and fitness levels while growing your professional network. The next opportunity might be between sets.</p><div className="mt-8 flex flex-wrap gap-2">{networkRoles.map((role) => <span key={role} className="rounded-full border border-[#15200e]/20 px-3 py-2 text-xs font-bold uppercase tracking-wider">{role}</span>)}</div></ScrollReveal><ScrollReveal className="grid grid-cols-2 gap-3 sm:grid-cols-4">{[Network, Brain, Users, Award].map((Icon, index) => <div key={index} className="flex aspect-square flex-col justify-between rounded-2xl bg-[#15200e]/10 p-4"><Icon className="h-6 w-6" /><span className="text-4xl font-black">0{index + 1}</span></div>)}</ScrollReveal></div></section>

      <section className="mx-auto max-w-7xl px-5 py-24 sm:px-8 lg:px-12"><div className="grid gap-16 lg:grid-cols-2"><ScrollReveal><p className="eyebrow">Start where you are</p><h2 className="section-title">Everyone has a <span>place here.</span></h2><div className="mt-10 space-y-3">{[['Beginner', 'A patient on-ramp, simple movement coaching, and the confidence to keep going.'], ['Intermediate', 'Structured progression, fresh challenges, and the support to break through plateaus.'], ['Advanced', 'Higher-level programming, performance goals, and a community that matches your ambition.']].map(([level, detail], index) => <div key={level} className="flex gap-4 border-t border-white/10 py-5"><span className="text-sm font-black text-[#d8f36b]">0{index + 1}</span><div><h3 className="font-bold">{level}</h3><p className="mt-1 text-sm text-white/55">{detail}</p></div></div>)}</div></ScrollReveal><ScrollReveal className="relative overflow-hidden rounded-3xl bg-[#101b18] p-8 sm:p-12"><div className="absolute right-0 top-0 h-64 w-64 rounded-full bg-[#d8f36b]/10 blur-3xl" /><div className="relative"><p className="eyebrow">Grow Fit experts</p><h2 className="mt-4 text-4xl font-black uppercase leading-none">The room is full of <span className="text-[#d8f36b]">teachers.</span></h2><p className="mt-5 text-white/55">Guest trainers, nutrition experts, coaches, and community leaders who make progress feel possible.</p><div className="mt-8 space-y-3 text-sm text-white/75">{['Monthly fitness talks', 'Live demonstrations', 'Friendly fitness competitions', 'Expert Q&A sessions'].map((item) => <div key={item} className="flex items-center gap-3"><Check className="h-4 w-4 text-[#d8f36b]" />{item}</div>)}</div></div></ScrollReveal></div></section>

      <section className="border-y border-white/10 bg-[#101b18] px-5 py-24 sm:px-8 lg:px-12"><div className="mx-auto grid max-w-7xl items-center gap-12 lg:grid-cols-2"><ScrollReveal className="order-2 overflow-hidden rounded-3xl lg:order-1"><img src="https://images.unsplash.com/photo-1504150558240-0b4fd8946624?auto=format&fit=crop&w=1200&q=80" alt="Parent and child enjoying a healthy outdoor activity" className="h-[420px] w-full object-cover" /></ScrollReveal><ScrollReveal className="order-1 lg:order-2"><p className="eyebrow">For the whole family</p><h2 className="section-title">Strong habits <span>start young.</span></h2><p className="mt-5 max-w-lg text-white/55">A polished, welcoming space for families to make movement part of how they live, not another item on the calendar.</p><div className="mt-8 grid gap-3 sm:grid-cols-2">{['Fun fitness activities', 'Weekend kids workouts', 'Family workout sessions', 'Healthy lifestyle education'].map((item) => <div key={item} className="rounded-xl border border-white/10 bg-white/[.04] p-4 text-sm font-bold">{item}</div>)}</div><button onClick={openModal} className="mt-8 inline-flex items-center gap-2 rounded-full bg-[#d8f36b] px-5 py-3 text-sm font-black text-[#15200e]">Explore family fitness <ArrowUpRight className="h-4 w-4" /></button></ScrollReveal></div></section>

      <section className="mx-auto max-w-7xl px-5 py-24 sm:px-8 lg:px-12"><ScrollReveal><div className="flex flex-col justify-between gap-6 md:flex-row md:items-end"><div><p className="eyebrow">The movement is growing</p><h2 className="section-title">Find your <span>people.</span></h2></div><p className="max-w-md text-white/55">Small wins become a different kind of powerful when they are witnessed, shared, and celebrated.</p></div></ScrollReveal><div className="mt-12 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{communityPillars.map((pillar, index) => <ScrollReveal key={pillar}><div className="group flex min-h-32 flex-col justify-between rounded-2xl border border-white/10 bg-white/[.04] p-5 transition hover:border-[#d8f36b]/50 hover:bg-[#d8f36b] hover:text-[#15200e]"><span className="text-xs font-bold text-[#d8f36b] group-hover:text-[#15200e]/60">0{index + 1}</span><span className="text-lg font-black uppercase leading-none">{pillar}</span></div></ScrollReveal>)}</div></section>

      <section className="px-5 pb-24 sm:px-8 lg:px-12"><div className="mx-auto max-w-7xl overflow-hidden rounded-3xl bg-[#d8f36b] px-6 py-14 text-center text-[#15200e] sm:px-12"><HeartPulse className="mx-auto h-8 w-8" /><h2 className="mt-5 text-4xl font-black uppercase leading-none sm:text-6xl">Your strongest chapter starts now.</h2><p className="mx-auto mt-5 max-w-xl text-[#15200e]/70">Join the 90-Day Challenge and make your next 90 days count.</p><button onClick={openModal} className="mt-8 inline-flex items-center gap-2 rounded-full bg-[#15200e] px-6 py-4 text-sm font-black uppercase tracking-wider text-white hover:bg-black">Join the 90-Day Challenge <ArrowUpRight className="h-4 w-4" /></button></div></section>
    </main>
  </>;
}
